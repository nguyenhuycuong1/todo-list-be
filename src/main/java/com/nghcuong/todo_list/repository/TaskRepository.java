...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query(value = "SELECT t.* FROM task t WHERE t.due_date = :dueDate AND t.user_id = :userId", nativeQuery = true)
    List<Task> findTasksByDueDate(@Param("dueDate") LocalDate dueDate, @Param("userId") Long userId);

    @Query(value = "SELECT t.* FROM task t WHERE t.priority = :priority AND t.user_id = :userId", nativeQuery = true)
    List<Task> findTasksByPriority(@Param("priority") Priority priority, @Param("userId") Long userId);

    @Query(value = "SELECT t.* FROM task t WHERE t.status = :status AND t.user_id = :userId", nativeQuery = true)
